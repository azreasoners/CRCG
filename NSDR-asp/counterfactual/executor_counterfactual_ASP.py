...
class Executor():
    ...
    def run(self, pg, answer_facts, c_idx,debug=False):
        
        exe_stack = []       

        num_of_objects=len(self.modules['objects']['func']())   
        obj_attributes=[]
        for i in range(num_of_objects):
            obj_attributes.append(self.sim.get_static_attrs(i))
        
        program_information=[]
        answer=answer_facts[c_idx]
            ...
            if m not in ['<START>']:
                if m not in self.modules:
                    ...
                    if step_output == 'error':
                        ...
                        return 'error',answer, program_information
                    exe_stack.append(step_output)
                    if debug:
                        print('> %s%s' % (m, argv))
                        print(exe_stack)
            program_information.append([m,argv,step_output])

        return str(exe_stack[-1]),answer, program_information


    def get_removed(self, pg,debug=False):
                ...
                if m not in self.modules:
                    ...
                    for i in range(self.modules[m]['nargs']):
                        if exe_stack:
                            ...
                            return 'error'
                    step_output = self.modules[m]['func'](*argv)
                    if m == 'filter_counterfact':
                        return argv[-1]
                    if step_output == 'error':
                        return 'error',obj_attributes,program_information,answer
                    exe_stack.append(step_output)
                    if debug:
                        print('> %s%s' % (m, argv))
                        print(exe_stack)
    ...
    def counterfact_events(self, remove_obj):
        ...
        if remove_obj not in self.all_objs:
            return 'error'
        if remove_obj not in self.sim.cf_events.keys():
            # print('---------------------')
            return 'error'
