...
for dd_idx,dd in enumerate(pbar):
    ...
    if pred!='tbd':
        ...
        GPT_prompt = get_GPT_prompt(cf_type, desc, desc_q_words, question, perc_q, False)
        GPT_response, prompt_cache, GPT_used1 = get_response_check(GPT_prompt,prompt_cache)
        response_text = GPT_response['choices'][0]['message']['content'].strip()
