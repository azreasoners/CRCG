...
for processing_index in range(int(sys.argv[1]), int(sys.argv[1])+1):
    ...
    for object_index, identity in enumerate(object_dict.keys()):
        ...
        x0.data[object_index] = torch.tensor(object_dict[identity]['initial_location']).to(device) #changed to .data next 4 lines 
        v0.data[object_index] = torch.tensor(object_dict[identity]['initial_velocity']).to(device)
    ...
    orig_traj['x']=x.detach().tolist()
    orig_traj['v']=v.detach().tolist()
    orig_traj['angle']=angle.detach().tolist()
    orig_traj['mass']=mass.detach().tolist()
    orig_traj['restitution'] = restitution.detach().tolist()

    output_dict['orig_traj']=orig_traj
    ...
    json.dump(output_dict, open(f'data/frame_info/frameInfo_{processing_index:05d}.json', 'w'))
