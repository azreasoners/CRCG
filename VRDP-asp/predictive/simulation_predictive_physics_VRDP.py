...
class Simulation():
    ...
    def __init__(self, ann_path, frame_params, n_vis_frames=128, use_event_ann=True, use_IOD=False, use_PM=False):
        ...
        if use_IOD:
            with open(os.path.join('../data/object_updated_results_ep_ts_gtc',ann_path[-14:])) as f:
    ...
    def _init_sim(self):
        ...
        if self.use_PM:
            ...
            collision_list,attr_to_idx, obj_motion_dict, predicted = physics.simulate(self.objs,p['trajectory'],self.collisions,self.frame_params,post_vid_only=True)
            self.obj_motion_dict=obj_motion_dict
