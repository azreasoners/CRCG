...
parser.add_argument('--ASP_CRCG_approx', action='store_true', help = "Use CRCG^{approx}.") 
...
for ann_idx in pbar:
    ...
    sim = Simulation(ann_path, frame_params, use_event_ann=1, use_IOD=args.IOD)
    exe = Executor(sim)
    valid_q_idx = 0
    ...
    for q_idx, q in enumerate(question_scene['questions']):
        ...
        if args.ASP_CRCG or args.ASP_CRCG_approx:
            simulated_ASP = json.load(open(f'../data/ASP_SIM_results/simulatedASP_{file_idx:05d}_{sim_idx}.json'))
    
            if len(simulated_ASP)==1 and (args.ASP_CRCG or args.ASP_CRCG_approx): 
        ...
        for c_idx, c in enumerate(q_ann['choices']):
            ...
            if answer_facts[c_idx] != 'tbd':
                ...
                if args.ASP_PE:
                    ...
                    pred, counterfactual_answer, program_information = exe.run(full_pg, {i:'tbd' for i in range(10)}, c_idx, debug=False)
                    pred = pred_map[pred]
